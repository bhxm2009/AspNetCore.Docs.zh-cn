...
### <a name="custom-authorizationmessagehandler-class"></a><span data-ttu-id="1dac8-124">自定义 `AuthorizationMessageHandler` 类</span><span class="sxs-lookup"><span data-stu-id="1dac8-124">Custom `AuthorizationMessageHandler` class</span></span>
...
## <a name="typed-httpclient"></a><span data-ttu-id="1dac8-143">类型化 `HttpClient`</span><span class="sxs-lookup"><span data-stu-id="1dac8-143">Typed `HttpClient`</span></span>
...
<span data-ttu-id="1dac8-172">`Program.Main` (`Program.cs`):</span><span class="sxs-lookup"><span data-stu-id="1dac8-172">`Program.Main` (`Program.cs`):</span></span>
...
## <a name="cross-origin-resource-sharing-cors"></a><span data-ttu-id="1dac8-180">跨域资源共享 (CORS)</span><span class="sxs-lookup"><span data-stu-id="1dac8-180">Cross-origin resource sharing (CORS)</span></span>
...
## <a name="handle-token-request-errors"></a><span data-ttu-id="1dac8-193">处理令牌请求错误</span><span class="sxs-lookup"><span data-stu-id="1dac8-193">Handle token request errors</span></span>
...
* <span data-ttu-id="1dac8-208">请求成功，应用拥有有效的令牌。</span><span class="sxs-lookup"><span data-stu-id="1dac8-208">The request succeeds, and the app has a valid token.</span></span>
...
<span data-ttu-id="1dac8-255">如果选择这样做，则允许将 UI 分成不同的页面。</span><span class="sxs-lookup"><span data-stu-id="1dac8-255">You're allowed to break the UI into different pages if you choose to do so.</span></span>
...
## <a name="customize-the-user"></a><span data-ttu-id="1dac8-264">自定义用户</span><span class="sxs-lookup"><span data-stu-id="1dac8-264">Customize the user</span></span>
...
* <span data-ttu-id="1dac8-326">服务器可以存储刷新令牌，并确保应用不会失去对第三方资源的访问权限。</span><span class="sxs-lookup"><span data-stu-id="1dac8-326">The server can store refresh tokens and ensure that the app doesn't lose access to third-party resources.</span></span>
...
<span data-ttu-id="1dac8-335">ID 令牌中的声明列表针对 v2.0 终结点会发生更改。</span><span class="sxs-lookup"><span data-stu-id="1dac8-335">The list of claims in the ID token changes for v2.0 endpoints.</span></span> <span data-ttu-id="1dac8-336">有关详细信息，请参阅[为什么要更新到 Microsoft 标识平台 (v2.0)？](/azure/active-directory/azuread-dev/azure-ad-endpoint-comparison)。</span><span class="sxs-lookup"><span data-stu-id="1dac8-336">For more information, see [Why update to Microsoft identity platform (v2.0)?](/azure/active-directory/azuread-dev/azure-ad-endpoint-comparison).</span></span>

## <a name="configure-and-use-grpc-in-components"></a><span data-ttu-id="1dac8-337">在组件中配置和使用 gRPC</span><span class="sxs-lookup"><span data-stu-id="1dac8-337">Configure and use gRPC in components</span></span>

<span data-ttu-id="1dac8-338">若要将 Blazor WebAssembly 应用配置为使用 [ASP.NET Core gRPC 框架](xref:grpc/index)：</span><span class="sxs-lookup"><span data-stu-id="1dac8-338">To configure a Blazor WebAssembly app to use the [ASP.NET Core gRPC framework](xref:grpc/index):</span></span>

* <span data-ttu-id="1dac8-339">在服务器上启用 gRPC-Web。</span><span class="sxs-lookup"><span data-stu-id="1dac8-339">Enable gRPC-Web on the server.</span></span> <span data-ttu-id="1dac8-340">有关详细信息，请参阅 <xref:grpc/browser>。</span><span class="sxs-lookup"><span data-stu-id="1dac8-340">For more information, see <xref:grpc/browser>.</span></span>
* <span data-ttu-id="1dac8-341">为应用的消息处理程序注册 gRPC 服务。</span><span class="sxs-lookup"><span data-stu-id="1dac8-341">Register gRPC services for the app's message handler.</span></span> <span data-ttu-id="1dac8-342">下面的示例将应用的授权消息处理程序配置为使用 [gRPC 教程中的 `GreeterClient` 服务](xref:tutorials/grpc/grpc-start#create-a-grpc-service) (`Program.Main`)：</span><span class="sxs-lookup"><span data-stu-id="1dac8-342">The following example configures the app's authorization message handler to use the [`GreeterClient` service from the gRPC tutorial](xref:tutorials/grpc/grpc-start#create-a-grpc-service) (`Program.Main`):</span></span>
...
<span data-ttu-id="1dac8-343">占位符 `{APP ASSEMBLY}` 是应用的程序集名称（例如 `BlazorSample`）。</span><span class="sxs-lookup"><span data-stu-id="1dac8-343">The placeholder `{APP ASSEMBLY}` is the app's assembly name (for example, `BlazorSample`).</span></span> <span data-ttu-id="1dac8-344">将 `.proto` 文件放在托管的 Blazor 解决方案的 `Shared` 项目中。</span><span class="sxs-lookup"><span data-stu-id="1dac8-344">Place the `.proto` file in the `Shared` project of the hosted Blazor solution.</span></span>

<span data-ttu-id="1dac8-345">客户端应用中的组件可使用 gRPC 客户端 (`Pages/Grpc.razor`) 发出 gRPC 调用：</span><span class="sxs-lookup"><span data-stu-id="1dac8-345">A component in the client app can make gRPC calls using the gRPC client (`Pages/Grpc.razor`):</span></span>
...
<span data-ttu-id="1dac8-346">占位符 `{APP ASSEMBLY}` 是应用的程序集名称（例如 `BlazorSample`）。</span><span class="sxs-lookup"><span data-stu-id="1dac8-346">The placeholder `{APP ASSEMBLY}` is the app's assembly name (for example, `BlazorSample`).</span></span> <span data-ttu-id="1dac8-347">若要使用 `Status.DebugException` 属性，请使用 [Grpc.Net.Client](https://www.nuget.org/packages/Grpc.Net.Client) 版本 2.30.0 或更高版本。</span><span class="sxs-lookup"><span data-stu-id="1dac8-347">To use the `Status.DebugException` property, use [Grpc.Net.Client](https://www.nuget.org/packages/Grpc.Net.Client) version 2.30.0 or later.</span></span>

<span data-ttu-id="1dac8-348">有关详细信息，请参阅 <xref:grpc/browser>。</span><span class="sxs-lookup"><span data-stu-id="1dac8-348">For more information, see <xref:grpc/browser>.</span></span>

## <a name="build-a-custom-version-of-the-authenticationmsal-javascript-library"></a><span data-ttu-id="1dac8-349">生成 Authentication.MSAL JavaScript 库的自定义版本</span><span class="sxs-lookup"><span data-stu-id="1dac8-349">Build a custom version of the Authentication.MSAL JavaScript library</span></span>

<span data-ttu-id="1dac8-350">如果应用需要[适用于 JavaScript 的 Microsoft 身份验证库 (MSAL.js)](https://www.npmjs.com/package/@azure/msal-browser) 的自定义版本，请执行以下步骤：</span><span class="sxs-lookup"><span data-stu-id="1dac8-350">If an app requires a custom version of the [Microsoft Authentication Library for JavaScript (MSAL.js)](https://www.npmjs.com/package/@azure/msal-browser), perform the following steps:</span></span>

1. <span data-ttu-id="1dac8-351">确认系统具有最新的开发人员 .NET SDK，或者从 [.NET Core SDK：安装程序和二进制文件](https://github.com/dotnet/installer#installers-and-binaries)获取最新的开发人员 SDK 并进行安装。</span><span class="sxs-lookup"><span data-stu-id="1dac8-351">Confirm the the system has the latest developer .NET SDK or obtain and install the latest developer SDK from [.NET Core SDK: Installers and Binaries](https://github.com/dotnet/installer#installers-and-binaries).</span></span> <span data-ttu-id="1dac8-352">此方案不需要配置内部 NuGet 源。</span><span class="sxs-lookup"><span data-stu-id="1dac8-352">Configuration of internal NuGet feeds isn't required for this scenario.</span></span>
1. <span data-ttu-id="1dac8-353">设置 `dotnet/aspnetcore` GitHub 存储库，以按照[从源生成 ASP.NET Core](https://github.com/dotnet/aspnetcore/blob/main/docs/BuildFromSource.md) 中的文档进行开发。</span><span class="sxs-lookup"><span data-stu-id="1dac8-353">Set up the `dotnet/aspnetcore` GitHub repository for development per the docs at [Build ASP.NET Core from Source](https://github.com/dotnet/aspnetcore/blob/main/docs/BuildFromSource.md).</span></span> <span data-ttu-id="1dac8-354">分叉和克隆或下载 [dotnet/aspnetcore GitHub 存储库](https://github.com/dotnet/aspnetcore)的 ZIP 存档。</span><span class="sxs-lookup"><span data-stu-id="1dac8-354">Fork and clone or download a ZIP archive of the [dotnet/aspnetcore GitHub repository](https://github.com/dotnet/aspnetcore).</span></span>
1. <span data-ttu-id="1dac8-355">打开 `src/Components/WebAssembly/Authentication.Msal/src/Interop/package.json` 文件，并设置所需的 `@azure/msal-browser` 版本。</span><span class="sxs-lookup"><span data-stu-id="1dac8-355">Open the the `src/Components/WebAssembly/Authentication.Msal/src/Interop/package.json` file and set the desired version of `@azure/msal-browser`.</span></span> <span data-ttu-id="1dac8-356">有关已发布版本列表，请访问 [`@azure/msal-browser` npm 网站](https://www.npmjs.com/package/@azure/msal-browser)，然后选择“版本”选项卡。</span><span class="sxs-lookup"><span data-stu-id="1dac8-356">For a list of released versions, visit the [`@azure/msal-browser` npm website](https://www.npmjs.com/package/@azure/msal-browser) and select the **Versions** tab.</span></span>
1. <span data-ttu-id="1dac8-357">使用命令行界面中的 `yarn build` 命令在 `src/Components/WebAssembly/Authentication.Msal/src` 文件夹中生成 `Authentication.Msal` 项目。</span><span class="sxs-lookup"><span data-stu-id="1dac8-357">Build the `Authentication.Msal` project in the `src/Components/WebAssembly/Authentication.Msal/src` folder with the `yarn build` command in a command shell.</span></span>
1. <span data-ttu-id="1dac8-358">如果应用使用[压缩资产 (Brotli/Gzip)](xref:blazor/host-and-deploy/webassembly#compression)，则压缩 `Interop/dist/Release/AuthenticationService.js` 文件。</span><span class="sxs-lookup"><span data-stu-id="1dac8-358">If the app uses [compressed assets (Brotli/Gzip)](xref:blazor/host-and-deploy/webassembly#compression), compress the `Interop/dist/Release/AuthenticationService.js` file.</span></span>
1. <span data-ttu-id="1dac8-359">如果生成了文件，请将该文件的 `AuthenticationService.js` 文件和压缩版本 (`.br`/`.gz`) 从 `Interop/dist/Release` 文件夹复制到应用的已发布资产中应用的 `publish/wwwroot/_content/Microsoft.Authentication.WebAssembly.Msal` 文件夹中。</span><span class="sxs-lookup"><span data-stu-id="1dac8-359">Copy the `AuthenticationService.js` file and compressed versions (`.br`/`.gz`) of the file, if produced, from the `Interop/dist/Release` folder into the app's `publish/wwwroot/_content/Microsoft.Authentication.WebAssembly.Msal` folder in the app's published assets.</span></span>

## <a name="additional-resources"></a><span data-ttu-id="1dac8-360">其他资源</span><span class="sxs-lookup"><span data-stu-id="1dac8-360">Additional resources</span></span>

* <xref:blazor/security/webassembly/graph-api>
* [<span data-ttu-id="1dac8-361">具有提取 API 请求选项的 `HttpClient` 和 `HttpRequestMessage`</span><span class="sxs-lookup"><span data-stu-id="1dac8-361">`HttpClient` and `HttpRequestMessage` with Fetch API request options</span></span>](xref:blazor/call-web-api#httpclient-and-httprequestmessage-with-fetch-api-request-options)
