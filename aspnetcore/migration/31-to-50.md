...
## <a name="changes-to-blazor-app-routing-logic-in-501-and-further-5x-releases-up-to-60"></a>Blazor在5.0.1 和更高版本中对应用路由逻辑的更改，最多可达6。0
...
>
> 设置 `PreferExactMatches` 为 `@false` 或将其保留为未设置的功能 *仅用于向后兼容*。
>
> 当发布 .NET 6 时，路由器将始终首选完全匹配项，并且该 `PreferExactMatches` 选项将不可用。

## <a name="update-blazor-webassembly-and-blazor-server-projects"></a>更新 Blazor WebAssembly 和 Blazor Server 项目
...
## <a name="update-blazor-webassembly-projects"></a>更新 Blazor WebAssembly 项目
...
1. 在 `Program.Main` (`Program.cs`) 中， `<app>` 通过向 CSS 选择器添加哈希，将该元素的引用更改为 CSS 选择器 `#` ：
...
1. 在 `Program.Main` (`Program.cs`) 中，将默认暂时性 `HttpClient` 注册更改为范围（如果存在）：
...
1. 在 `Program.Main` (`Program.cs` **`Client`** 托管解决方案的应用) Blazor ：
    
   * （可选）替换 `builder.HostEnvironment.BaseAddress` 字符串客户端基址。
   * 将任何命名的临时客户端工厂注册更改为范围。
   ...
   在前面的代码中， `{APP NAMESPACE}` 占位符是应用的命名空间。

### <a name="standalone-blazor-webassembly-app-with-microsoft-accounts"></a>Blazor WebAssemblyMicrosoft 帐户的独立应用
...
### <a name="standalone-blazor-webassembly-app-with-azure-active-directory-aad"></a>Blazor WebAssembly具有 Azure Active Directory (AAD) 的独立应用
...
### <a name="standalone-blazor-webassembly-app-with-azure-active-directory-aad-b2c"></a>Blazor WebAssembly (AAD) B2C 的独立应用 Azure Active Directory
...
### <a name="hosted-blazor-webassembly-app-with-azure-active-directory-aad-or-aad-b2c"></a>Blazor WebAssembly具有 Azure Active Directory (AAD) 或 AAD B2C 的托管应用
...
### <a name="update-the-server-project-of-a-hosted-blazor-solution"></a>更新托管解决方案的服务器项目 Blazor
...
## <a name="update-a-blazor-progressive-web-application-pwa"></a>Blazor (PWA 更新渐进式 Web 应用程序) 
...
## <a name="update-razor-class-libraries-rcls"></a>Razor (RCLs) 更新类库
...
## <a name="model-binding-changes-in-aspnet-core-mvc-and-razor-pages"></a>ASP.NET Core MVC 和页面中的模型绑定更改 Razor
